...
<form action="{{route('flyff.install.setupDatabase')}}" method="POST">
    ...
    <div class="mb-3">
        ...
        @error('sqlsrv_port')
            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
        @enderror
        <small id="sqlsrv_port_info" class="form-text">If you use <code>DESKTOP-XXX</code> (Windows authentification like SQLServer Studio) leave the port empty, otherwise the default port is <code>1443</code> </small>
    </div>
